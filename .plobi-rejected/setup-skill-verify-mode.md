# Verify/Check Mode for `plobi-scaffold`

This project will not add a dedicated verify/check mode (or a separate verify skill) for `plobi-scaffold`.

## Why this is out of scope

A second skill — or a `--verify` flag — for checking whether `docs/agents/*.md` artifacts still match the seed-template schema would duplicate work the existing scaffold skill already handles in conversation.
