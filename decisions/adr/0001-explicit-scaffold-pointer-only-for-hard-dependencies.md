# Explicit `/plobi-scaffold` pointer only for hard dependencies
...
- **Hard dependency** (`plobi-slice`, `plobi-synthesize`) — include an explicit one-liner: _"… should have been provided to you — run `/plobi-scaffold` if not."_ Without the mapping, output is wrong, not just fuzzy.
